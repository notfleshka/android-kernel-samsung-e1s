...
void dp_reg_phy_mode_setting(struct dp_cal_res *cal_res)
{
	...
	case PDIC_NOTIFY_DP_PIN_A:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_A\n");
		break;
	case PDIC_NOTIFY_DP_PIN_B:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_B\n");
		break;
	case PDIC_NOTIFY_DP_PIN_C:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_C\n");
		fallthrough;
	case PDIC_NOTIFY_DP_PIN_E:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_E\n");
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
	...
	case PDIC_NOTIFY_DP_PIN_D:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_D\n");
		fallthrough;
	case PDIC_NOTIFY_DP_PIN_F:
		cal_log_info(0, "PDIC_NOTIFY_DP_PIN_F\n");
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
...
{
	if (index == 0) {
		return (int*)phy_eq_hbr0_1;
	} else {
		if (exynos_soc_info.main_rev == 0)
			return (int*)phy_eq_hbr2_3_evt0;
		else
			return (int*)phy_eq_hbr2_3;
	}
}
#endif
