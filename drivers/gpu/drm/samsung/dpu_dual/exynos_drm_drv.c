	...
	for_each_new_crtc_in_state(old_state, crtc, new_crtc_state, i) {
		...
		const size_t num_planes =
			hweight32(new_crtc_state->plane_mask &
				  ~exynos_crtc->rcd_plane_mask);
		int num_special_plane = priv->num_special_plane;
		int j;
		...
		if (new_crtc_state->planes_changed) {
			for (j = num_planes; j < max_planes - num_special_plane; j++) {
		...
		struct drm_atomic_state *old_state)
{
	...
	for_each_oldnew_crtc_in_state(old_state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		if (!ret) {
			...
			if (mcd_drm_check_commit_skip(to_exynos_crtc(crtc), __func__)) {
...
static int exynos_drm_bind(struct device *dev)
{
...
#if IS_ENABLED(CONFIG_DRM_MCD_COMMON)
	mcd_drm_init_no_disp(no_display, no_display1);
#endif
	...
	win_cnt = get_plane_num(drm) - private->num_special_plane;
