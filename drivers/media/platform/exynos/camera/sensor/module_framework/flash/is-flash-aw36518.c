...
extern int aw36518_create_sysfs(struct class*);
#endif
...
static int sensor_aw36518_set_torch_pre_config(struct is_flash *flash, u32 intensity)
{
	int ret = 0;
...
int flash_aw36518_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FLASH_SET_FIRE:
		...
		if (mode == CAM2_FLASH_MODE_OFF) {
			...
			if (!flash->use_pre_config_work)
				ret = sensor_aw36518_set_torch_pre_config(flash, ctrl->value);
			ret = aw36518_fled_mode_ctrl(
				AW36518_FLED_MODE_TORCH_FLASH, 0);
			if (ret)
	...
	case V4L2_CID_FLASH_SET_TORCH_PRE_CONFIG:
		...
		ret = sensor_aw36518_set_torch_pre_config(flash, ctrl->value);
		if (ret) {
				err("%s torch pre-flash on fail", __func__);
		}
		break;
	default:
...
static int flash_aw36518_probe(struct device *dev, struct i2c_client *client)
{
	...
	aw36518_create_sysfs(camera_class);
#endif
