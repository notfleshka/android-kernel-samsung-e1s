...
static int __init flash_s2mu107_platform_probe_i2c(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev;
...
static int __init is_sensor_flash_s2mu107_init_i2c(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_flash_s2mu107_platform_driver,
				flash_s2mu107_platform_probe_i2c);
	if (ret)
...
late_initcall_sync(is_sensor_flash_s2mu107_init_i2c);
