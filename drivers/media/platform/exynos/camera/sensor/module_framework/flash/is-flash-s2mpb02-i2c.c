...
static int sensor_s2mpb02_set_torch_pre_config(struct is_flash *flash, u32 intensity)
{
	int ret = 0;
	enum aa_ae_flashmode aeflashMode = flash->flash_data.aeflashMode;
	u32 firingPower = flash->flash_data.firingPower;

	dbg_flash("[%s] use_pre_config_work(%d), aeflashMode(%d), firingPower(%d), intensity(%d)\n",
		__func__, flash->use_pre_config_work, aeflashMode, firingPower, intensity);
	...
	case AA_FLASHMODE_ON_ALWAYS: /*TORCH mode*/
		...
		if (firingPower == 5)
			ret = s2mpb02_set_torch_current(S2MPB02_TORCH_RECORDING, LEDS_S2MPB02_ADAPTIVE_MOVIE_CURRENT);
		else
#endif
			ret = s2mpb02_set_torch_current(S2MPB02_TORCH_RECORDING, 0);
		break;
	case AA_FLASHMODE_ON:
		ret = s2mpb02_set_torch_current(S2MPB02_TORCH_PREFLASH, 0);
		break;
...
static int sensor_s2mpb02_main_flash_on(struct is_flash *flash, u32 intensity)
{
	int ret = 0;
...
static int sensor_s2mpb02_flash_control(struct v4l2_subdev *subdev, enum flash_mode mode, u32 intensity)
{
	...
	if (!flash->use_pre_config_work)
		sensor_s2mpb02_set_torch_pre_config(flash, intensity);
	...
	if (mode == CAM2_FLASH_MODE_OFF) {
		...
		ret = sensor_s2mpb02_main_flash_on(flash, intensity);
		if (ret)
...
int flash_s2mpb02_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FLASH_SET_INTENSITY:
		...
		dbg_flash("[%s] V4L2_CID_FLASH_SET_INTENSITY : ctrl->value(%d)\n", __func__, ctrl->value);
		if (ctrl->value < 0) {
	...
	case V4L2_CID_FLASH_SET_FIRING_TIME:
		...
		dbg_flash("[%s] V4L2_CID_FLASH_SET_FIRING_TIME : ctrl->value(%d)\n", __func__, ctrl->value);
		if (ctrl->value < 0) {
	...
	case V4L2_CID_FLASH_SET_FIRE:
		dbg_flash("[%s] V4L2_CID_FLASH_SET_FIRE : ctrl->value(%d)\n", __func__, ctrl->value);
	...
	case V4L2_CID_FLASH_SET_TORCH_PRE_CONFIG:
		...
		ret = sensor_s2mpb02_set_torch_pre_config(flash, ctrl->value);
		if (ret) {
			err("sensor_s2mpb02_set_torch(mode:%d, val:%d) is fail(%d)",
					(int)flash->flash_data.mode, ctrl->value, ret);
			goto p_err;
		}
		break;
