...
static int sensor_bu64981_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, val_high, val_low);
...
int sensor_bu64981_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, 0x80, 0x00);
	if (ret < 0)
		goto p_err_mutex;

	ret = actuator->ixc_ops->addr8_write8(client, 0xCD, 0x91);
	if (ret < 0)
		goto p_err_mutex;
...
static int sensor_bu64981_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_bu64981_driver = {
	.probe  = sensor_bu64981_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
