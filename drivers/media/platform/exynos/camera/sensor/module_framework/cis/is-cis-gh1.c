...
int sensor_gh1_cis_set_pdxtc_calibration(struct is_cis *cis)
{
	...
	is_sec_get_rom_info(&rom_info, ROM_ID_FRONT);
	cal_buf = rom_info->buf;
#ifdef CAMERA_GH1_CAL_MODULE_VERSION
	info("[%s] cur_header : %s", __func__, rom_info->header_ver);
	if (rom_info->header_ver[10] < CAMERA_GH1_CAL_MODULE_VERSION)
#endif
	{
		info("[%s] skip calibration, cal value not valid", __func__);
		return 0;
	}
	if (rom_info->rom_pdxtc_cal_data_start_addr == -1) {
		err("[%s] cal addr empty");
		goto p_err;
	} else {
		cal_data = &cal_buf[rom_info->rom_pdxtc_cal_data_start_addr];
	}

	cal_size[0] = rom_info->rom_pdxtc_cal_data_0_size;
	if (cal_size[0] == -1) {
		err("[%s] cal_size0 empty");
		goto p_err;
	}
	cal_size[1] = rom_info->rom_pdxtc_cal_data_1_size;
	if (cal_size[1] == -1) {
...
int sensor_gh1_cis_set_xtc_calibration(struct is_cis *cis)
{
	...
	if (rom_info->rom_xtc_cal_data_start_addr == -1) {
		err("[%s] cal_addr empty");
		goto p_err;
	} else {
		cal_data = &cal_buf[rom_info->rom_xtc_cal_data_start_addr];
	}
