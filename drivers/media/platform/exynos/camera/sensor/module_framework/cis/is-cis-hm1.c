...
int sensor_hm1_cis_set_cal(struct v4l2_subdev *subdev)
{
	...
	if (rom_info->header_ver[10] < CAMERA_HM1_CAL_MODULE_VERSION) {
		start_addr = rom_info->rom_xtc_cal_data_addr_list[HM1_CAL_START_ADDR];
		...
		len = (rom_info->rom_xtc_cal_data_addr_list_len / HM1_CAL_ROW_LEN) - 1;
		if (len >= 0) {
			end_addr = rom_info->rom_xtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_END_ADDR];
			if (end_addr >= 7) {
	...
	info("[%s] PDXTC start\n", __func__);
	start_addr = rom_info->rom_pdxtc_cal_data_addr_list[HM1_CAL_START_ADDR];
	...
	for (len = 0; len < rom_info->rom_pdxtc_cal_data_addr_list_len / HM1_CAL_ROW_LEN; len++) {
		...
		if (len != 0)
			start_addr = rom_info->rom_pdxtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_START_ADDR];
		end_addr = rom_info->rom_pdxtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_END_ADDR];

#ifdef HM1_BURST_WRITE
		if (rom_info->rom_pdxtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_BURST_CHECK]) {
	...
	info("[%s] GCC start\n", __func__);
	start_addr = rom_info->rom_gcc_cal_data_addr_list[HM1_CAL_START_ADDR];
	...
	for (len = 0; len < rom_info->rom_gcc_cal_data_addr_list_len / HM1_CAL_ROW_LEN; len++) {
		...
		if (len != 0)
			start_addr = rom_info->rom_gcc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_START_ADDR];
		end_addr = rom_info->rom_gcc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_END_ADDR];

#ifdef HM1_BURST_WRITE
		if (rom_info->rom_gcc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_BURST_CHECK]) {
			cal_size = (end_addr - start_addr) / 2 + 1;
	...
	for (len = 0; len < rom_info->rom_xtc_cal_data_addr_list_len / HM1_CAL_ROW_LEN; len++) {
		...
		if (len != 0)
			start_addr = rom_info->rom_xtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_START_ADDR];
		end_addr = rom_info->rom_xtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_END_ADDR];

#ifdef HM1_BURST_WRITE
		if (rom_info->rom_xtc_cal_data_addr_list[len * HM1_CAL_ROW_LEN + HM1_CAL_BURST_CHECK]) {
			cal_size = (end_addr - start_addr) / 2 + 1;
