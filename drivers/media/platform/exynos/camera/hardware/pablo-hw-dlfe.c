...
static int pablo_hw_dlfe_dbg_set(const char *val, const struct kernel_param *kp)
{
	int ret = 0, argc = 0;
	char **argv;
...
static int pablo_hw_dlfe_dbg_get(char *buffer, const struct kernel_param *kp)
{
	int ret;
	u32 dbg_type;

	ret = sprintf(buffer, "= DLFE DEBUG Configuration =====================\n");
	...
	for (dbg_type = 1; dbg_type < DLFE_DBG_TYPE_NUM; dbg_type++)
		ret += sprintf(buffer + ret, "  - %10s[%3s]: echo %d <en> %s> debug_dlfe\n",
...
static const struct kernel_param_ops pablo_hw_dlfe_dbg_ops = {
	.set = pablo_hw_dlfe_dbg_set,
	.get = pablo_hw_dlfe_dbg_get,
};

module_param_cb(debug_dlfe, &pablo_hw_dlfe_dbg_ops, NULL, 0644);
...
{
	return pablo_common_ctrl_init(&GET_HW(hw_ip)->pcc, hw_ip->pmio, hw_ip->name, PCC_M2M, NULL);
}
