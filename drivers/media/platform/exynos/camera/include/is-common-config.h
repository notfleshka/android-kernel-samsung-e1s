...
#else
#define sfrinfo(fmt, args...) info_wo_memlog("[SFR]", fmt, ##args)
#endif
...
#define info_wo_memlog(prefix, fmt, args...) is_info("[@]" prefix fmt, ##args)
...
#define info_adt(fmt, args...) \
	info_common("[ADT]", fmt, ##args)
#define warn_adt(fmt, args...) \
	warn_common("[ADT][WRN]%d:", fmt "\n", __LINE__, ##args)
#define err_adt(fmt, args...) \
	err_common("[ADT][ERR]%s:%d:", fmt "\n", __func__, __LINE__, ##args)
