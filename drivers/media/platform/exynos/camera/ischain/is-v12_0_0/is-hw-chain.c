...
void is_hw_s2mpu_cfg(void)
{
	...
	reg = ioremap(0x1A1C0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* CSTAT: SYSMMU_D_CSTAT_S2 */
	reg = ioremap(0x1A960054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* BYRP: SYSMMU_BRP_S0_S2 */
	reg = ioremap(0x1B850054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* RGBP: SYSMMU_RGBP_S0_S2 */
	reg = ioremap(0x1B050054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MCFP/MCSC/DLFE: SYSMMU_MCFP_S0_S2 */
	reg = ioremap(0x1F890054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* YUVP/SHRP: SYSMMU_YUVP_S0_S2 */
	reg = ioremap(0x1C090054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* LME/GDC: SYSMMU_LME_S0_S2 */
	reg = ioremap(0x288C0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* DLNE */
	reg = ioremap(0x28080054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);
}
...
void is_hw_check_psm_state(struct is_frame *frame,
	struct is_dvfs_ctrl *dvfs_ctrl, struct is_group *group)
{
	int vendor;
	bool psm = false;
	struct is_dvfs_psm_mode *psm_mode = &dvfs_ctrl->psm_mode;

	vendor = (dvfs_ctrl->dvfs_scenario >> IS_DVFS_SCENARIO_VENDOR_SHIFT) &
		 IS_DVFS_SCENARIO_VENDOR_MASK;

	psm = (vendor == IS_DVFS_SCENARIO_VENDOR_PSM) ? true : false;
	psm_mode->changed = 0;
	if (psm_mode->psm != psm) {
		mgrdbgs(1, "psm state changed(%d -> %d)\n", group, group, frame,
			psm_mode->psm, psm);
		psm_mode->psm = psm;
		psm_mode->changed = 1;
	}
