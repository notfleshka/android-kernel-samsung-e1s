...
static void slsi_lbm_traffic_mon_netif_napi_cb(void *client_ctx, u32 state, u32 tput_tx, u32 tput_rx);
#endif
...
static uint saturation_trigger_threshold = 50;
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
static uint threaded_napi_cpus_default = 0xFFFFFFFF;
module_param(threaded_napi_cpus_default, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(threaded_napi_cpus_default, "default threaded napi cpus");

static uint threaded_napi_cpus_high = 0x000001FF;
module_param(threaded_napi_cpus_high, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(threaded_napi_cpus_high, "high threaded napi cpus");
#endif
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
void push_netif_napi_event(struct net_device *dev, u32 state, enum netif_napi_reason reason)
{
	struct ctrl_event *event;

	event = kmalloc(sizeof(*event), GFP_ATOMIC);
	if (!event) {
		SLSI_NET_ERR(dev, "netif NAPI event cannot be allocated: state %u\n", state);
		return;
	}

	/* If state < TRAFFIC_MON_CLIENT_STATE_LOW, event is triggered by LATENCY_CRT_DATA ioctl command */
	if (reason == NETIF_NAPI_REASON_LATENCY)
		SLSI_NET_DBG3(dev, SLSI_LBM, "PUSH netif NAPI event Latency mode %u\n", state);
	else
		SLSI_NET_DBG3(dev, SLSI_LBM, "PUSH netif NAPI event tput state %u\n", state);
	event->type = NAPI_T;
	event->event.napi.dev = dev;
	event->event.napi.state = state;
	event->event.napi.reason = reason;
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
static int slsi_netdev_napi_affinity_set(struct napi_struct *napi, u32 cpus)
{
	struct task_struct *task = napi->thread;
	cpumask_var_t mask;
	int cpu;
	int ret = -ENOMEM;

	if (!task) {
		SLSI_ERR_NODEV("NAPI thread is null\n");
		return -EINVAL;
	}
...
static void lbm_ctrl_work_func(struct work_struct *data)
{
	...
	while (1) {
		...
		case NAPI_T:
		{
			struct net_device *dev = event->event.napi.dev;
			struct netdev_vif *ndev_vif = netdev_priv(event->event.napi.dev);
			u32 state = event->event.napi.state;
			enum netif_napi_reason reason = event->event.napi.reason;

			SLSI_DBG3_NODEV(SLSI_LBM, "POP netif NAPI event reason %d, state %u\n", reason, state);
			/*
			 * 1. netif NAPI disabled if 'LATENCY_CRT_DATA' is set to a value greater than 0.
			 * 2. netif NAPI disabled if throughput is low.
			 */
			...
			if (reason == NETIF_NAPI_REASON_LATENCY) {
				if (state)
					SLSI_NET_INFO(dev, "netif NAPI disabled(latency mode: %d)\n", state);
				else
					SLSI_NET_INFO(dev, "netif NAPI enabled(latency mode: %d)\n", state);
				ndev_vif->use_netif_napi = !state;
			} else {
				if (state == TRAFFIC_MON_CLIENT_STATE_LOW)
					SLSI_NET_INFO(dev, "netif NAPI disabled(tput state: %d)\n", state);
				else
					SLSI_NET_INFO(dev, "netif NAPI enabled(tput state: %d)\n", state);
				ndev_vif->use_netif_napi = state;
			}
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
			/* NAPI gro list should be flushed before switching NAPI instance */
			if (ndev_vif->gro_enabled == SLSI_GRO_ENABLED && !ndev_vif->use_netif_napi)
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
			/* Bind threaded NAPI to selected cpus */
			if (event->event.rps.state == TRAFFIC_MON_CLIENT_STATE_LOW) {
				if (slsi_netdev_napi_affinity_set(&ndev_vif->rx_data_napi, threaded_napi_cpus_default))
					SLSI_NET_INFO(dev, "Low tput: threaded napi binded to all cpus:%x\n",
						      threaded_napi_cpus_default);
			} else {
				if (slsi_netdev_napi_affinity_set(&ndev_vif->rx_data_napi, threaded_napi_cpus_high))
					SLSI_NET_INFO(dev, "High tput: threaded napi binded to performance cpus:%x\n",
						      threaded_napi_cpus_high);
			}
#endif
		}
		break;
...
#ifdef CONFIG_SCSC_WLAN_CPUHP_MONITOR
	cpumask_t cluster_mask;
	int cluster_id = 0;
	int cpu_first = -1, cpu_num = 0;
	int core_idx, related_idx;
	struct cpufreq_policy *cpucl;
#endif
	load_man.sdev = sdev;
#ifdef CONFIG_SCSC_WLAN_CPUHP_MONITOR
	cpumask_clear(&cluster_mask);

	for_each_online_cpu(core_idx) {
		if (cpumask_test_cpu(core_idx, &cluster_mask)) {
			continue;
		}

		/* Get CPU cluster information */
		cpucl = cpufreq_cpu_get(core_idx);
		if (!cpucl) {
			SLSI_ERR_NODEV("Cpufreq policy not found for CPU(%d)\n", core_idx);
			continue;
		}
		...
		/* Set cluster info */
		for_each_cpu(related_idx, cpucl->related_cpus) {
			if (related_idx >= SLSI_NR_CPUS) {
				SLSI_ERR_NODEV("CPU(%d) not exist, skip assign\n", related_idx);
				continue;
			}
			if (cpu_first == -1) {
				cpu_first = related_idx;
			}
			cpu_num++;
			sdev->cpu_cluster_map[related_idx] = cluster_id;
			cpumask_set_cpu(related_idx, &cluster_mask);
		}

		cl_info[cluster_id].first_cpu = cpu_first;
		cl_info[cluster_id].cpu_num = cpu_num;

		cluster_id++;
		cpufreq_cpu_put(cpucl);
	}

	sdev->cluster_num = cluster_id;
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
	/* For netif napi enable/disable according to throughput */
	ndev_vif->napi_info = slsi_lbm_register_netif_napi_control(dev);
#endif
	return 0;
}
...
{
#if IS_ENABLED(CONFIG_SCSC_WLAN_RX_NAPI_GRO) && !defined(CONFIG_SCSC_WLAN_NAPI_PER_NETDEV)
	int i;
#endif
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
	if (ndev_vif->napi_info) {
		SLSI_INFO_NODEV("Unregister netif NAPI control\n");
		slsi_lbm_unregister_netif_napi_control(dev);
	}
#endif
	return 0;
}
					 ...
					 int irq, int idx)
{
	...
	for (i = 0; i < SLSI_NR_CPUS; i++) {
		pinfo = &bh_tmp->bh_priv.napi.cpu_info[i];
		napi_ins = &bh_tmp->bh_priv.napi.cpu_info[i].napi_instance;
		clear_bit(0, bh_tmp->bh_priv.napi.cpu_info[i].is_queued);
		bh_tmp->bh_priv.napi.cpu_info[i].csd.func = trigger_napi;
		bh_tmp->bh_priv.napi.cpu_info[i].csd.info = pinfo;
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
struct napi_ctrl_info *slsi_lbm_register_netif_napi_control(struct net_device *dev)
{
	struct netdev_vif	*ndev_vif = netdev_priv(dev);
	struct napi_ctrl_info	*napi_handle;

	if (ndev_vif->napi_info) {
		SLSI_NET_DBG4(dev, SLSI_LBM, "NAPI info controller is already set(ifnum:%d)\n", ndev_vif->ifnum);
		return ndev_vif->napi_info;
	}

	napi_handle = kmalloc(sizeof(*napi_handle), GFP_ATOMIC);

	if (!napi_handle) {
		SLSI_ERR_NODEV("Cannot allocate NAPI ctrl structure\n");
		return NULL;
	}

	SLSI_DBG3_NODEV(SLSI_LBM, "Register netif napi controller for ifnum:%d\n", ndev_vif->ifnum);
	ndev_vif->napi_info = napi_handle;
	ndev_vif->napi_info->mid = napi_threshold_in_mbps[TH_MID] * MBPS;
	ndev_vif->napi_info->high = napi_threshold_in_mbps[TH_HIGH] * MBPS;
	ndev_vif->napi_info->state = TRAFFIC_MON_CLIENT_STATE_LOW;
	if (slsi_traffic_mon_client_register(ndev_vif->sdev,
								       dev,
								       TRAFFIC_MON_CLIENT_MODE_EVENTS,
								       ndev_vif->napi_info->mid,
								       ndev_vif->napi_info->high,
								       TRAFFIC_MON_DIR_DEFAULT,
								       slsi_lbm_traffic_mon_netif_napi_cb)) {
		SLSI_NET_WARN(dev, "failed to register netif NAPI controller cb\n");
		kfree(ndev_vif->napi_info);
		ndev_vif->napi_info = NULL;
	}
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
void slsi_lbm_unregister_netif_napi_control(struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct ctrl_event *event, *tmp;
	...
	SLSI_NET_DBG3(dev, SLSI_LBM, "Unregister netif NAPI controller for ifnum:%d\n", ndev_vif->ifnum);
	slsi_traffic_mon_client_unregister(ndev_vif->sdev, dev);

	spin_lock_bh(&load_man.ctrl_event_list_lock);
	list_for_each_entry_safe(event, tmp, &load_man.ctrl_event_list, list) {
		if (event->type == NAPI_T && event->event.napi.dev == dev) {
			list_del(&event->list);
			kfree(event);
		}
	}
...
static void trigger_napi(void *data)
{
	struct napi_cpu_info *napi_cpu_info = (struct napi_cpu_info *)data;

	clear_bit(0, napi_cpu_info->is_queued);
	SLSI_DBG4_NODEV(SLSI_LBM, "schedule NAPI in changed CPU\n");
	napi_schedule(&napi_cpu_info->napi_instance);
}
...
static int slsi_lbm_napi_poll(struct napi_struct *napi, int budget)
{
	...
	if (bh_s->cpu_affinity) {
		...
		if (current_cpu != target_cpu) {
			struct napi_cpu_info *target_cpu_info = &bh_s->bh_priv.napi.cpu_info[target_cpu];

			napi_complete_done(napi, 0);
			if (idx < NP_TX_0)
				SLSI_DBG3_NODEV(SLSI_LBM, "CPU affinity was changed, move to CPU%d\n", target_cpu);

			if (!test_and_set_bit(0, target_cpu_info->is_queued))
				smp_call_function_single_async(target_cpu, &target_cpu_info->csd);
			return 0;
		}
	}
	...
	if (bh_s->io_saturation && bh_s->cpu_affinity) {
		...
		if (work_done >= budget) {
			 ...
			 * 1. continuous work done 64 count > saturation_trigger_threshold (50)
			 * 2. continuous work done 64 more than 1 second in a row
			 */
			if (bh_s->io_saturation->saturation_cnt == 1) {
				if (!bh_s->override_timer_start) {
					bh_s->override_timer_start = true;
					bh_s->start_time = jiffies;
				}
			}

			napi_saturated = (current_cpu < cpu_high && !bh_s->io_saturation->saturated &&
					  bh_s->io_saturation->saturation_cnt > saturation_trigger_threshold);
			timeout_triggered = jiffies - bh_s->start_time > msecs_to_jiffies(SLSI_OVERRIDE_TIMEOUT_IN_MS);

			if (napi_saturated || timeout_triggered) {
				if (napi_saturated)
					SLSI_DBG3_NODEV(SLSI_LBM, "NAPI is saturated, move to override state\n");
				else
					SLSI_DBG3_NODEV(SLSI_LBM, "Timeout triggered, move to override state\n");
				bh_s->io_saturation->saturation_cnt = 0;
				bh_s->io_saturation->saturated = true;
				bh_s->override_timer_start = false;
	...
	/* keep polling to see if new pakcets are pending */
	if (work_done && work_done < budget)
		work_done = budget;
...
static void slsi_lbm_traffic_mon_netif_napi_cb(void *client_ctx, u32 state, u32 tput_tx, u32 tput_rx)
{
	...
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct napi_ctrl_info *napi_handle = ndev_vif->napi_info;
	u32 old_state;

	old_state = napi_handle->state;
	...
	/*
	* old state : MID or higer, new state : LOW  → Don't use netif NAPI
	* old state : LOW, new state : MID or higher → Use netif NAPI
	*/
	if ((state == TRAFFIC_MON_CLIENT_STATE_LOW) != (old_state == TRAFFIC_MON_CLIENT_STATE_LOW)) {
		napi_handle->state = state;
		push_netif_napi_event(dev, state, NETIF_NAPI_REASON_TPUT);
	}
}
#endif
