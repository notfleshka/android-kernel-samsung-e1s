...
static int sap_mlme_notifier(struct slsi_dev *sdev, unsigned long event)
{
	...
	case SCSC_WIFI_SUSPEND:
		...
		if (!(sdev->device_config.user_suspend_mode) || (sdev->device_config.host_state & SLSI_HOSTSTATE_SUSPEND_DISALLOWED)) {
	...
	case SCSC_WIFI_RESUME:
		/* resume sap_mlme in resume_work */
		break;
...
#error "CONFIG_SCSC_WLAN_TAS cannot be enabled if CONFIG_SLSI_WLAN_DCTAS is enabled."
#endif
	case MLME_SAR_IND:
		dctas_wlan_sar_ind(sdev, skb);
		break;
#endif
#if defined(CONFIG_SCSC_WLAN_TAS)
	case MLME_SAR_IND:
	...
	case MLME_SET_BSS_MAX_CHANNEL_WIDTH_IND:
		...
		break;
	case MLME_LINK_STATE_INFO_IND:
		slsi_rx_link_state_info_ind(sdev, dev, skb);
		break;
	default:
		kfree_skb(skb);
...
static int sap_mlme_rx_handler(struct slsi_dev *sdev, struct sk_buff *skb)
{
	...
	/* the MLME control plane can't deal with SKB with frag_list, so make
	 * the SKB linear if the SKB has fragments
	 */
	if (skb_has_frag_list(skb)) {
		...
		if (!skb2) {
			SLSI_WARN(sdev, "error allocating skb %d bytes\n", skb->len);
			kfree_skb(skb);
			return 0;
		}
		if (skb->len)
			fapi_append_data(skb2, skb->data, skb_headlen(skb));
		skb_walk_frags(skb, frag_skb) {
			if (frag_skb->len)
				fapi_append_data(skb2, frag_skb->data, frag_skb->len);
		}
...
void sap_mlme_resume(struct slsi_dev *sdev)
{
