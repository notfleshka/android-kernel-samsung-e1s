...
void slsi_dev_resume_work(struct work_struct *work)
{
	struct slsi_dev *sdev = container_of(work, struct slsi_dev, resume_work);

	slsi_hip_resume_wrapper(sdev);
	sap_mlme_resume(sdev);

	slsi_wake_unlock(&sdev->wlan_wl_resume);
}
...
struct slsi_dev *slsi_dev_attach(struct device *dev, struct scsc_mx *core, struct scsc_service_client *mx_wlan_client)
{
	...
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_resume, "wlan_resume");
	...
	if (slsi_hip_pre_allocate_hip_priv(sdev) != 0) {
		SLSI_ERR(sdev, "fail to allocate hip_priv\n");
		goto err_hip_init;
	}

	if (slsi_udi_node_init(sdev, dev) != 0) {
		SLSI_ERR(sdev, "failed to init UDI\n");
		goto err_hip_priv_init;
	}
	...
	sdev->cat2_dsi_id = 20; /* DSI index Mid mode */
#endif
...
#endif
	wlbt_register_panel_state_notifier(sdev);

#ifdef CONFIG_SLSI_WLAN_DCTAS
	init_dctas(sdev);
#endif
...
#endif
	INIT_WORK(&sdev->resume_work, slsi_dev_resume_work);
...
err_if:
	...
	slsi_wake_lock_destroy(&sdev->wlan_wl_resume);
...
void slsi_dev_detach(struct slsi_dev *sdev)
{
...
#ifdef CONFIG_SCSC_WIFI_NAN_ENABLE
#if (KERNEL_VERSION(5, 12, 0) <= LINUX_VERSION_CODE)
	if (!list_empty(&sdev->nan_data_interface_create_delete_data)) {
	...
	cancel_work_sync(&sdev->nan_data_interface_create_delete_work);
#endif
#endif
	slsi_stop(sdev);
	...
	for (j = 0; j < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; j++)
		kfree(sdev->ini_conf_struct.doubleindexmib[j].ini_conf_buff);
#endif
...
#if defined(CONFIG_SLSI_WLAN_CP_COEX)
	wlbt_unregister_ril_notifier(sdev);
#endif
	wlbt_unregister_panel_state_notifier(sdev);
	...
	for (j = 0; j < SLSI_MAX_RADIO_SUPPORTED; j++) {
		kfree(sdev->radio_stat[j]);
		sdev->radio_stat[j] = NULL;
	}
}
...
int __init slsi_dev_load(void)
{
	...
	SLSI_INFO_NODEV("No wlbt device\n");
	return 0;
#endif
