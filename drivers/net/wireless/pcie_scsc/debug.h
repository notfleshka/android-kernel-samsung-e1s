...
#define SLSI_BIN_TAG_DEBUG(tag, start, len)         do { if (slsi_dbg_lvl_all) SCSC_BIN_TAG_DEBUG(tag, start, len); } while (0)
...
#define SLSI_BIN_TAG_DEBUG(tag, start, len)         do {} while (0)
...
#undef SLSI_DBG3
#undef SLSI_DBG4
#define SLSI_DBG1(sdev, filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG1_SDEV(sdev, filter, fmt, ## arg); } while (0)
#define SLSI_DBG2(sdev, filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG2_SDEV(sdev, filter, fmt, ## arg); } while (0)
#define SLSI_DBG3(sdev, filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG3_SDEV(sdev, filter, fmt, ## arg); } while (0)
#define SLSI_DBG4(sdev, filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG4_SDEV(sdev, filter, fmt, ## arg); } while (0)

#undef SLSI_NET_DBG1
#undef SLSI_NET_DBG2
#undef SLSI_NET_DBG3
#undef SLSI_NET_DBG4
#define SLSI_NET_DBG1(ndev, filter, fmt, arg ...)	do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG1_NDEV(ndev, filter, fmt, ## arg); } while (0)
#define SLSI_NET_DBG2(ndev, filter, fmt, arg ...)	do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG2_NDEV(ndev, filter, fmt, ## arg); } while (0)
#define SLSI_NET_DBG3(ndev, filter, fmt, arg ...)	do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG3_NDEV(ndev, filter, fmt, ## arg); } while (0)
#define SLSI_NET_DBG4(ndev, filter, fmt, arg ...)	do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG4_NDEV(ndev, filter, fmt, ## arg); } while (0)

#undef SLSI_DBG1_NODEV
#undef SLSI_DBG2_NODEV
#undef SLSI_DBG3_NODEV
#undef SLSI_DBG4_NODEV
#define SLSI_DBG1_NODEV(filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG1(filter, fmt, ## arg); } while (0)
#define SLSI_DBG2_NODEV(filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG2(filter, fmt, ## arg); } while (0)
#define SLSI_DBG3_NODEV(filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG3(filter, fmt, ## arg); } while (0)
#define SLSI_DBG4_NODEV(filter, fmt, arg ...)		do { if (slsi_dbg_lvl_all) SCSC_TAG_DBG4(filter, fmt, ## arg); } while (0)

#undef SLSI_BIN_TAG_DEBUG
#define SLSI_BIN_TAG_DEBUG(tag, start, len)         do { if (slsi_dbg_lvl_all) SCSC_BIN_TAG_DEBUG(tag, start, len); } while (0)

void slsi_dbg_on_off(bool enable);
