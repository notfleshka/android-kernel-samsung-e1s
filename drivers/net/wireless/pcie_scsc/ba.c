...
static void slsi_rx_ba_free_buffer(struct net_device *dev, struct slsi_peer *peer, int tid)
{
	...
	/* Reset the buffer before returning to the pool. */
	if (peer) {
		slsi_spinlock_lock(&ndev_vif->ba_lock);
		peer->ba_session_rx[tid]->dev = NULL;
		peer->ba_session_rx[tid]->peer = NULL;
		peer->ba_session_rx[tid]->buffer_size = 0;
		peer->ba_session_rx[tid]->occupied_slots = 0;
		peer->ba_session_rx[tid]->start_sn = 0;
		peer->ba_session_rx[tid]->expected_sn = 0;
		peer->ba_session_rx[tid]->highest_received_sn = 0;
		peer->ba_session_rx[tid]->closing = false;
		peer->ba_session_rx[tid]->trigger_ba_after_ssn = false;
		peer->ba_session_rx[tid]->tid = 0;
		peer->ba_session_rx[tid]->ba_timeouts = 0;
		peer->ba_session_rx[tid]->ba_drops_old = 0;
		peer->ba_session_rx[tid]->ba_drops_replay = 0;
						...
						struct slsi_ba_session_rx *ba_session_rx, u16 sn, struct slsi_ba_frame_desc *frame_desc, bool *stop_timer)
{
	...
	*stop_timer = false;
	...
	if (((sn - ba_session_rx->expected_sn) & 0xFFF) <= BA_WINDOW_BOUNDARY) {
		...
		/* If driver gets MSDU with SN == SSN or window start advances past SSN,
		 * driver stops remembering last SN passed to upper layers and stops applying
		 * additional checks
		 */
		if (ba_session_rx->last_sn_check) {
			if (sn == ba_session_rx->start_sn) {
				SLSI_NET_DBG4(dev, SLSI_RX_BA, "stop last SN check: tid=%d, sn=%d, ssn=%d\n", ba_session_rx->tid, sn, ba_session_rx->start_sn);
				ba_session_rx->last_sn_check = false;
			}
			if (((ba_session_rx->expected_sn - ba_session_rx->start_sn) & 0xFFF) > ba_session_rx->buffer_size) {
				SLSI_NET_DBG4(dev, SLSI_RX_BA, "stop last SN check: tid=%d, sn=%d, expected_sn=%d, ssn=%d\n", ba_session_rx->tid, sn, ba_session_rx->expected_sn, ba_session_rx->start_sn);
				ba_session_rx->last_sn_check = false;
			}
		}
		...
		if (sn == ba_session_rx->expected_sn) {
			...
			if (ba_session_rx->last_sn_check) {
				u16 new_expected_sn;
				u16 last_sn = peer->last_sn[ba_session_rx->tid];

				/* if driver gets MSDU with SN == last SN passed to upper layers + 1 then
				 * driver passes to upper layers and updates last SN passed to upper layers
				 */
				SLSI_NET_DBG4(dev, SLSI_RX_BA, "last SN check: sn = %d, last_sn = %d\n", sn, last_sn);
				if (sn == ((last_sn + 1) & 0xFFF)) {
					SLSI_NET_DBG4(dev, SLSI_RX_BA, "last SN check: sn == last_sn + 1\n");
					new_expected_sn = ((sn + 1) & 0xFFF);
					ba_update_expected_sn(dev, ba_session_rx, new_expected_sn);
					ba_add_frame_to_ba_complete(dev, ba_session_rx, frame_desc);
					peer->last_sn[ba_session_rx->tid] = sn;
					return -1;
				}
			}
...
#endif
{
	...
	if (!dev) {
		SLSI_ERR_NODEV("dev is NULL, Invalid timer active. Del the timer\n");
		return;
	}
...
void slsi_ba_store_sn(struct net_device *dev, struct slsi_peer *peer, struct sk_buff *skb)
{
	struct slsi_skb_cb *skb_cb;

	skb_cb = (struct slsi_skb_cb *)skb->cb;
	if (skb_cb->tid <= FAPI_PRIORITY_QOS_UP7) {
		SLSI_NET_DBG4(dev, SLSI_RX_BA, "store: tid=%d sn=%d\n",
			skb_cb->tid,
			skb_cb->seq_num);
		peer->last_sn[skb_cb->tid] = skb_cb->seq_num;
	}
}
			    ...
			    u16 tid, u16 buffer_size, u16 start_sn)
{
	...
	ba_session_rx->last_sn_check = true;
