...
void slsi_regd_use_monitor(struct slsi_dev *sdev)
{
	struct ieee80211_regdomain *slsi_monitor_regdom_custom = sdev->device_config.domain_info.regdomain;
	struct ieee80211_reg_rule  reg_rules_monitor[] = {
		REG_RULE(2400, 2483.5, 40, 0, 20, 0),
		REG_RULE(5150, 5875, 160, 0, 20, 0),
		REG_RULE(5955, 6275, 320, 0, 20, 0),
		REG_RULE(6435, 6755, 320, 0, 20, 0),
		REG_RULE(6835, 7115, 160, 0, 20, 0),
	};
	int	i;

	SLSI_DBG1_NODEV(SLSI_INIT_DEINIT, "regulatory monitor mode\n\n");
	slsi_monitor_regdom_custom->n_reg_rules = (sizeof(reg_rules_monitor)) / sizeof(reg_rules_monitor[0]);
	for (i = 0; i < slsi_monitor_regdom_custom->n_reg_rules; i++)
		slsi_monitor_regdom_custom->reg_rules[i] = reg_rules_monitor[i];

	slsi_monitor_regdom_custom->alpha2[0] = '0';
	slsi_monitor_regdom_custom->alpha2[1] = '0';
}

void slsi_regd_init(struct slsi_dev *sdev)
{
	if (sdev->monitor_mode)
		slsi_regd_use_monitor(sdev);
...
int slsi_read_regulatory(struct slsi_dev *sdev)
{
	...
	char *reg_file_t_legacy = "slsi_reg_database.bin";
#else
