...
struct slsi_skb_cb {
	...
	/* To determine if this skb is processed in netif NAPI instance or not */
	u8 is_netif_napi:1;
#endif
	bool wakeup;
	bool is_ciphered;
...
static inline char *slsi_strtok_r (char *str, const char *delim, char **saveptr) {
	char *end;
	...
	/* Terminate the token and make *saveptr next to it.*/
	*end = '\0';
	*saveptr = end + 1;
	return str;
}
