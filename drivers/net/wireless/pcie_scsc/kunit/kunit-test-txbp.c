...
static void test_slsi_txbp_get_dynamic_netdev_mod(struct kunit *test)
{
	struct net_device     *dev = TEST_TO_DEV(test);
	struct tx_netdev_data *tx_priv = kunit_kzalloc(test, sizeof(*tx_priv), GFP_KERNEL);
	u32 ndev_mod = 0;

	tx_priv->netdev_mod = bp_mod / txbp_priv.vif_cnt;
	ndev_mod = slsi_txbp_get_dynamic_netdev_mod(tx_priv);
	KUNIT_EXPECT_EQ(test, tx_priv->netdev_mod, ndev_mod);
}
...
static void test_slsi_txbp_napi(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_txbp_napi(&napi_cpu_info->napi_instance, 1024));
	bp_mod -= 5;
	txbp_priv.cod = prev;
	kfree_skb(skb);
}
...
static void test_slsi_txbp_immediate_fwd(struct kunit *test)
{
	struct tx_struct      *tx = kunit_kzalloc(test, sizeof(*tx), GFP_KERNEL);
	struct slsi_dev       *sdev = TEST_TO_SDEV(test);
	struct net_device     *dev = TEST_TO_DEV(test);
	struct netdev_vif     *ndev_vif = netdev_priv(dev);
	...
	skb = fapi_alloc(ma_unitdata_req, FAPI_SAP_TYPE_MA, 0, 128);
	skb->next = tx->assigned_q[0];
	skb->prev = tx->assigned_q[0];
	fapi_set_u16(skb, u.ma_unitdata_req.vif, 1);
	slsi_skb_cb_get(skb)->peer_idx = 2;
	...
	txbp_priv.bp_mod += SLSI_TX_BP_MBULK_GUARD;
	prev = txbp_priv.cod;
	bp_virtual_pressure[0] = 1;
	tx_bot_mvif_rules_enable = true;
	tx_bot_ac_cod_manual = true;

	((struct fapi_signal *)(skb)->data)->id = FAPI_SAP_TYPE_MLME;
	KUNIT_EXPECT_EQ(test, 0, slsi_txbp_immediate_fwd(dev, tx_priv));
	((struct fapi_signal *)(skb)->data)->id = FAPI_SAP_TYPE_MA;
	KUNIT_EXPECT_EQ(test, 1, slsi_txbp_immediate_fwd(dev, tx_priv));
	KUNIT_EXPECT_EQ(test, 0, slsi_txbp_immediate_fwd(dev, tx_priv));
...
static struct kunit_case txbp_test_cases[] = {
	...
	KUNIT_CASE(test_slsi_txbp_get_dynamic_netdev_mod),
	...
	KUNIT_CASE(test_slsi_txbp_immediate_fwd),
