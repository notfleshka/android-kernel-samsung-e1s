...
static void test_slsi_mlme_set_roaming_parameters(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -ENODEV, slsi_mlme_set_roaming_parameters(sdev, dev, psid, mib_value, mib_length));
...
static void test_slsi_mlme_set_keepalive_parameters(struct kunit *test)
{
	...
	ndev_vif->activated = true;
	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	ndev_vif->vifnum = 0x0001;
	keepalive_time = 30;

#ifdef 0
	sdev->sig_wait.cfm = fapi_alloc(mlme_set_keepalive_parameters_cfm, MLME_SET_KEEPALIVE_PARAMETERS_CFM, ndev_vif->vifnum, 10);
	fapi_set_u16(sdev->sig_wait.cfm, u.mlme_set_keepalive_parameters_cfm.result_code, FAPI_RESULTCODE_SUCCESS);
	ndev_vif->mgmt_tx_cookie = 0x1234;
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;
	SLSI_NET_INFO(dev, "Test 1\n");
...
static void test_slsi_mlme_get_mib_data(struct kunit *test)
{
	...
	struct netdev_vif                      *ndev_vif = netdev_priv(dev);
	struct slsi_dev                        *sdev = ndev_vif->sdev;
	static const struct slsi_mib_get_entry get_values[] = { { SLSI_PSID_UNIFI_RSSI_PER_RADIO, { 1, 0 }},
	...
	struct slsi_mib_value                  *values = kmalloc_array((size_t)psids_length,
								       sizeof(struct slsi_mib_value), GFP_KERNEL);
...
static void test_slsi_mlme_set_low_latency_mode_req(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test,
			-EIO,
			slsi_mlme_set_low_latency_mode_req(sdev, dev, latency_mode));
...
static void test_slsi_mlme_set_max_bw(struct kunit *test)
{
	...
	slsi_sig_send_init(&ndev_vif->sig_wait);
	sdev->mlme_blocked = true;
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_set_max_bw(sdev, dev, 0, 0, 80));

	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;
	ndev_vif->sig_wait.process_id = SLSI_TX_PROCESS_ID_MAX;
	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_set_max_bw(sdev, dev, 0, 1, 80));

	ndev_vif->sig_wait.process_id = SLSI_TX_PROCESS_ID_MAX -1;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_set_max_bw(sdev, dev, 0, 1, 80));
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
}

#if defined(CONFIG_SLSI_WLAN_CP_COEX)
static void test_slsi_mlme_set_cellular_info(struct kunit *test)
{
	struct net_device *dev = TEST_TO_DEV(test);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev *sdev = ndev_vif->sdev;
	...
	sdev->mlme_blocked = true;
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_set_cellular_info(sdev));

	// Need kunit_mock_wait_for_completion_timeout for MLME_SET_CELLULAR_INFO_REQ/CFM after fapi added.
#if 0
	slsi_sig_send_init(&ndev_vif->sig_wait);
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;
	ndev_vif->sig_wait.process_id = SLSI_TX_PROCESS_ID_MAX;
	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_set_cellular_info(sdev));

	ndev_vif->sig_wait.process_id = SLSI_TX_PROCESS_ID_MAX - 1;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_set_cellular_info(sdev));
#endif
}
#endif
...
#if defined(CONFIG_SLSI_WLAN_CP_COEX)
	KUNIT_CASE(test_slsi_mlme_set_cellular_info),
#endif
	{}
};
