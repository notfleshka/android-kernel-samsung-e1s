...
void slsi_pcie_shutdown_cb(void)
{
	struct slsi_dev *sdev;
	int i;
	struct netdev_vif *ndev_vif;

	SLSI_INFO_NODEV("Set pcie_shutdown start\n");
	sdev = cm_ctx.sdev;
	sdev->pcie_shutdown = true;
	...
	SLSI_INFO_NODEV("Set pcie_shutdown complete\n");
}
#endif
...
void slsi_sm_service_failed(struct slsi_dev *sdev, const char *reason, bool is_work)
{
	...
	state = atomic_read(&sdev->cm_if.cm_if_state);
	if ((state != SCSC_WIFI_CM_IF_STATE_STARTED && state != SCSC_WIFI_CM_IF_STATE_STOPPING) ||
	    sdev->pcie_shutdown) {
		mutex_unlock(&slsi_start_mutex);
...
#ifdef CONFIG_SCSC_PCIE_CHIP
	scsc_service_register_shutdown_cb(slsi_pcie_shutdown_cb);
#endif
...
exit:
#ifdef CONFIG_SCSC_PCIE_CHIP
	scsc_service_unregister_shutdown_cb();
#endif
	if (!r)
		sdev->service = NULL;
	mutex_unlock(&slsi_start_mutex);
