...
EXPORT_SYMBOL_GPL(aw36518_set_torch_current);
...
int aw36518_create_sysfs(struct class *class)
{
	...
	err = device_create_file(camera_flash_dev, &dev_attr_rear_flash);
	if (err < 0) {
	...
	/* creat reg attr */
	err = device_create_file(camera_flash_dev, &dev_attr_reg);
	if (err < 0)
		return err;

	return err;
}
EXPORT_SYMBOL(aw36518_create_sysfs);
