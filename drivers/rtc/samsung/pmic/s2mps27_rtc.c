...
#endif
static struct s2m_rtc_info *static_info;
...
int secure_debug_set_config(uint8_t addr, uint8_t config)
{
	...
	if (addr != S2MP_RTC_REG_SECURE1 && addr != S2MP_RTC_REG_SECURE2 &&
	    addr != S2MP_RTC_REG_SECURE3 && addr != S2MP_RTC_REG_SECURE4) {
	...
	ret = s2mps27_update_reg(static_info->i2c, addr, config, 0xF);
	if (ret < 0) {
		pr_err("%s: s2mps27_update_reg failed\n", __func__);
		return -1;
	}

	ret = s2mps27_read_reg(static_info->i2c, addr, &val);
	if (ret < 0) {
		pr_err("%s: s2mps27_read_reg failed\n", __func__);
		return -1;
	}
...
void secure_debug_write(uint8_t addr, uint8_t bit, uint8_t val)
{
	...
	ret = s2mps27_update_reg(static_info->i2c, addr, val << bit, 1 << bit);
	if (ret < 0)
		return;
}
...
void secure_debug_clear(void)
{
	...
	for (i = S2MP_RTC_REG_SECURE1; i <= S2MP_RTC_REG_SECURE4; i++) {
		ret = s2mps27_write_reg(static_info->i2c, i, 0x00);
		if (ret < 0)
			return;
	}
}
				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mps27_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mps27_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mps27_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mps27_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mps27_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
...
static int s2m_rtc_stop_boot_alarm0(struct s2m_rtc_info *info)
{
	...
	ret = s2mps27_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
	...
	ret = s2mps27_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_stop_boot_alarm(struct s2m_rtc_info *info)
{
	...
	ret = s2mps27_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
	...
	ret = s2mps27_bulk_write(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_read_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mps27_read_reg(info->pmic_i2c, reg, &val);
	if (ret < 0) {
		dev_err(info->dev, "%s: %d fail to read STATUS2 reg(%d)\n", __func__, __LINE__, ret);
		goto out;
	}
...
static int _s2m_rtc_set_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mps27_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &info->update_reg);
	if (ret < 0) {
	...
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, (char)info->update_reg);
	if (ret < 0)
...
static irqreturn_t s2m_rtc_boot_alarm_irq(int irq, void *data)
{
	...
	if (info->boot_alarm_enabled) {
		if (is_charging_mode)
			...
			info->update_reg &= ~RTC_WAKE_MASK;
			s2mps27_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, (char)info->update_reg);
		}
	}
...
//		ret = s2mps27_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
...
//	if (pdata->osc_xin >= 0) {
//		ret = s2mps27_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
...
static void exynos_smpl_warn_work(struct work_struct *work)
{
	...
	if (!state) {
		sec_pm_smpl_warn_throttle_by_one_step();
		queue_delayed_work(system_freezable_wq, &info->irq_work,
				msecs_to_jiffies(10));
	} else {
		dev_info(info->dev, "%s: SMPL_WARN polling End!\n", __func__);
		sec_pm_smpl_warn_unthrottle();
#if IS_ENABLED(CONFIG_SOC_EXYNOS9830_EVT0)
						...
						struct s2mps27_platform_data *pdata)
{
	...
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mps27_platform_data *pdata)
{
	...
	ret = s2mps27_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mps27_platform_data *pdata)
{
	...
	ret = s2mps27_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data_alrm1);
	if (ret < 0)
		return ret;
	...
	ret = s2mps27_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, info->update_reg);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write update reg(%d)\n",
			__func__, ret);
		return ret;
	...
	ret = s2mps27_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mps27_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mps27_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
				   ...
				   struct s2m_rtc_info *info, int irq_base)
{
	...
	switch (info->iodev->device_type) {
	case S2MPS27X:
		info->boot_alarm_irq = irq_base + S2MPS27_PMIC_IRQ_RTCA1_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
				 ...
				 struct s2m_rtc_info *info, int irq_base)
{
	...
	switch (info->iodev->device_type) {
	case S2MPS27X:
		info->irq = irq_base + S2MPS27_PMIC_IRQ_RTCA0_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mps27_platform_data *pdata = dev_get_platdata(iodev->dev);
	...
	/* Set secure debug */
	ret = secure_debug_set_config(S2MP_RTC_REG_SECURE1, RTC_BOOTING);
	if (ret < 0) {
...
static void s2m_rtc_shutdown(struct platform_device *pdev)
{
	/*disable wtsr, smpl */
	struct s2m_rtc_info *info = platform_get_drvdata(pdev);
	struct s2mps27_platform_data *pdata = dev_get_platdata(info->iodev->dev);
	...
	/* Set secure debug */
	if (secure_debug_set_config(S2MP_RTC_REG_SECURE1, RTC_SHUTDOWN) < 0)
		pr_err("%s: secure_debug_set_config failed\n", __func__);
}
...
MODULE_DEVICE_TABLE(platform, s2m_rtc_id);
...
static int s2mps27_rtc_suspend(struct device *dev)
{
	...
	/* Set secure debug */
	if (secure_debug_set_config(S2MP_RTC_REG_SECURE1, RTC_SUSPEND) < 0)
		pr_err("%s: secure_debug_set_config failed\n", __func__);
...
static int s2mps27_rtc_resume(struct device *dev)
{
	...
	/* Set secure debug */
	if (secure_debug_set_config(S2MP_RTC_REG_SECURE1, RTC_RESUME) < 0)
		pr_err("%s: secure_debug_set_config failed\n", __func__);
