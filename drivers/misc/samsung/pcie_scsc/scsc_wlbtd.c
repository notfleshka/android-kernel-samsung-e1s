...
static bool wlbtd_termination_flag = false;
...
static int msg_from_wlbtd_sable_cb(struct sk_buff *skb, struct genl_info *info)
{
	...
	input_string = (char *)nla_data(info->attrs[1]);
	...
	case SCSC_WLBTD_ERR_PARSE_FAILED:
		...
		if (strstr(input_string, "WLBTD process terminated") != NULL) {
			SCSC_TAG_INFO(WLBTD, "WLBTD termination is acknowledged\n");
			wlbtd_termination_flag = true;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
			mxman_park_recovery_thread();
#endif
		}
		break;
	case SCSC_WLBTD_FW_PANIC_TAR_GENERATED:
...
static int msg_from_wlbtd_init_cb(struct sk_buff *skb, struct genl_info *info)
{
	(void)skb;

	wlbtd_termination_flag = false;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	mxman_unpark_recovery_thread();
#endif
...
static const char* logCollectionDaemon[MAX_NUM_PROCESS] = {"moredump.bin", "wlbtd"};

static void kill_task_by_name(const char *name)
{
	...
	if (pid != -1) {
		SCSC_TAG_CRIT(WLBTD, "Send SIGKILL for %s(PID: %d)\n", name, pid);
		send_sig(SIGKILL, task, 0);
	} else {
		SCSC_TAG_CRIT(WLBTD, "Not find the pid of %s\n", name);
	}
}
#endif
int call_wlbtd_sable(u8 trigger_code, u16 reason_code, bool sable, bool moredump)
{
	...
	if (completion_jiffies) {
		...
		for (int i = 0; i < MAX_NUM_PROCESS; i++) {
			kill_task_by_name(logCollectionDaemon[i]);
		}
#else
		wlbtd_seq = (wlbtd_seq + 1) % (MAX_WLBTD_SEQ + 1);
#endif
...
#if !defined(CONFIG_WLBT_SPLIT_RECOVERY)
bool scsc_wlbtd_get_wlbtd_termination_flag(void)
{
	return wlbtd_termination_flag;
}
#endif
