...
static BLOCKING_NOTIFIER_HEAD(link_status_chain);
...
MODULE_PARM_DESC(check_l1ss_delay, "check_l1ss_delay us, default 1000us(1ms)");

static uint pmu_dump_delay = 1000;
module_param(pmu_dump_delay, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(pmu_dump_delay, "pmu_dump_delay ms, default 1000ms(1s)");
...
#endif
extern int exynos_pcie_rc_poweron_speed(int ch_num, int spd);
...
bool pcie_get_shutdown_flag(struct pcie_mif *pcie)
{
	return pcie->shutdown_flag;
}

void pcie_set_pcie_cb_panic(bool val)
{
	enable_pcie_cb_panic = val;
}
...
#define MAX_RSV_MEM 20 * 1024 * 1204

static void __iomem *pcie_mif_map_region(unsigned long phys_addr, size_t size, bool cacheable)
{
	size_t i;
	struct page **pages;
	void *vmem;
...
void *pcie_map(struct pcie_mif *pcie, size_t *allocated)
{
	if (allocated)
		*allocated = 0;
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	pcie->mem = pcie_mif_map_region(pcie->mem_start, SHARED_MEM1_SIZE, false);
	pcie->mem_2 = pcie_mif_map_region(pcie->mem_start + SHARED_MEM1_SIZE, SHARED_MEM2_SIZE, true);
#else
	pcie->mem = pcie_mif_map_region(pcie->mem_start, pcie->mem_sz, false);
#endif
	...
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Map: virt %p phys %lx\n", pcie->mem, (uintptr_t)pcie->mem_start);
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Map2: virt %p phys %lx\n", pcie->mem_2, (uintptr_t)pcie->mem_start + SHARED_MEM1_SIZE);
#endif

#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
	hsi_sysmmu_map(pcie->iommu_dom, pcie->mem_start, pcie->mem_start, pcie->mem_sz, 0);
#else
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Unmap: virt %p phys %lx\n", pcie->mem, (uintptr_t)pcie->mem_start);
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Unmap2: virt %p phys %lx\n", pcie->mem_2, (uintptr_t)pcie->mem_start + SHARED_MEM1_SIZE);
#endif
#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
	hsi_sysmmu_unmap(pcie->iommu_dom, pcie->mem_start, pcie->mem_sz);
#else
...
static void wlbt_pcie_linkdown_cb(struct exynos_pcie_notify *noti)
{
	...
	if (!pcie_is_initialized()) {
		SCSC_TAG_ERR(PCIE_MIF, "wlbt pcie module probe is not executed yet\n");
		return;
	}
	pcie = pci_get_drvdata((struct pci_dev *)noti->user);
	...
	/* delay for BT_PMU_DUMP */
	mdelay(pmu_dump_delay);
...
void __iomem *pcie_mif_map_bar(struct pci_dev *pdev, u8 index)
{
	...
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, &pdev->dev, "BAR%u vaddr=0x%p busaddr=%pad len=%u\n", index, vaddr, &busaddr,
			 (int)len);
#endif
	return vaddr;
}
...
void pcie_mif_register_shutdown_cb(struct pcie_mif *pcie, void (*shutdown_cb)(void))
{
	pcie->shutdown_cb = shutdown_cb;
}

void pcie_mif_unregister_shutdown_cb(struct pcie_mif *pcie)
{
	pcie->shutdown_cb = NULL;
}
...
static void pcie_mif_module_shutdown(struct pci_dev *pcie_dev)
{
	struct pcie_mif *pcie;
	...
	pcie = pci_get_drvdata(pcie_dev);

	SCSC_TAG_INFO(PCIE_MIF, "\n");

	/* Block handling any event in PCIe fsm thread */
	pcie->shutdown_flag = true;

	/* Only dislink PCIe When pcie link is on */
	if (get_pcie_link_state(pcie) == PCIE_LINK_STATE_ON) {
		pcie_mif_poweroff(pcie);
	}

	/* call service shutdown callback */
	if(pcie->shutdown_cb != NULL)
		pcie->shutdown_cb();
...
static struct pci_driver scsc_pcie = {
	...
	.shutdown = pcie_mif_module_shutdown,
};

void pcie_irq_reg_pmu_handler(struct pcie_mif *pcie, void (*handler)(int irq, void *data), void *dev)
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif pmu int handler %pS\n", handler);
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif pmu pcie_off int handler %pS\n", handler);
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif pmu pcie_error int handler %pS\n", handler);
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif wlan int handler %pS\n", handler);
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif wpan int handler %pS\n", handler);
...
{
	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "Registering mif recovery notifier %pS\n", notifier);
...
static void set_pcie_on(int state)
{
	...
	if(state != PCIE_LINK_STATE_ERROR)
		blocking_notifier_call_chain(&link_status_chain, state, NULL);
...
void pcie_mif_link_init(struct pcie_mif *pcie)
{
	/* set gen2 1lane as default */
	pcie->gen = 2;
	pcie->lane = 1;
}
EXPORT_SYMBOL(pcie_mif_link_init);
...
u32 pcie_mif_link_change(struct pcie_mif *pcie)
{
	...
	pcie->gen = readw(pcie->registers_ramrp + RAMRP_HOSTIF_GEN);
	pcie->lane = readw(pcie->registers_ramrp + RAMRP_HOSTIF_LANE);

	current_gen = exynos_pcie_rc_speed_check(pcie->ch_num);
	if (pcie->gen == current_gen) {
		SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Don't need to change speed (req: %d, cur: %d)\n", pcie->gen, current_gen);
		goto skip_gen_change;
	}
	ret = pcie_mif_speed_change(pcie->ch_num, pcie->gen);
	if (ret) {
...
skip_gen_change:
	current_lane = exynos_pcie_rc_lane_check(pcie->ch_num);
	if (pcie->lane == current_lane) {
		SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Don't need to change lane (req: %d, cur: %d)\n", pcie->lane, current_lane);
		goto done;
	}
	ret = pcie_mif_lane_change(pcie->ch_num, pcie->lane);
	if (ret) {
		msg = PMU_AP_MSG_LINK_CHANGE_ERROR;
		goto done;
	}
	...
	pcie_set_mbox_pmu_pcie(pcie, msg);
	return need_delay;
}
...
int pcie_mif_poweron(struct pcie_mif *pcie)
{
	...
	if (pcie_is_initialized()) {
		ret = exynos_pcie_rc_poweron_speed(pcie->ch_num, pcie->gen);
		if (ret) {
		...
		ret = pcie_mif_lane_change(pcie->ch_num, pcie->lane);
		if (ret) {
			SCSC_TAG_ERR(PCIE_MIF, "Error to change lane %d, ret = %d\n", pcie->lane, ret);
		}
		...
		ret = exynos_pcie_rc_poweron_speed(pcie->ch_num, MAX_GEN);
		if (ret) {
			set_pcie_on(PCIE_LINK_STATE_OFF);
			wake_unlock(&pcie->pcie_link_wakelock);
			mutex_unlock(&pci_link_lock);
...
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "register link_status_cb\n");
	return blocking_notifier_chain_register(&link_status_chain, nb);
}
EXPORT_SYMBOL(pcie_mif_register_link_status_notifier);

int pcie_mif_unregister_link_status_notifier(struct notifier_block *nb)
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "unregister link_status_cb\n");
	return blocking_notifier_chain_unregister(&link_status_chain, nb);
}
EXPORT_SYMBOL(pcie_mif_unregister_link_status_notifier);

#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
void pcie_mif_set_iommu_dom(struct pcie_mif *pcie, struct iommu_domain *iommu_dom)
{
	if (!pcie->iommu_dom)
		pcie->iommu_dom = iommu_dom;
}
EXPORT_SYMBOL(pcie_mif_set_iommu_dom);

struct iommu_domain *pcie_mif_get_iommu_dom(struct pcie_mif *pcie)
{
	return pcie->iommu_dom;
}
EXPORT_SYMBOL(pcie_mif_get_iommu_dom);
#endif
