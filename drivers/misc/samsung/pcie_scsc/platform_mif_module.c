...
static void platform_mif_module_probe_registered_clients(struct scsc_mif_abs *mif_abs)
{
	...
	struct mif_mmap_node *mif_mmap_node, *next_node;
	...
	list_for_each_entry_safe(mif_mmap_node, next_node, &mif_module.mif_mmap_list, list) {
		mif_mmap_node->driver->probe(mif_mmap_node->driver, mif_abs);
	}
}
