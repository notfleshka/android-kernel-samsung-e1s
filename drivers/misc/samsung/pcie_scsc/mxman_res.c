...
int mxman_res_delayed_wlan_fw_init(struct mxman *mxman, void *start_dram)
{
	int r;
	char *build_id;
	char *ttid;
	...
	/* Get log-string.bin for wlan subsystem */
...
int mxman_res_fw_init(struct mxman *mxman, struct fwhdr_if **fw_wlan, struct fwhdr_if **fw_wpan, void *start_dram,
		      size_t size_dram, bool hdm_wifi_enable)
#else
int mxman_res_fw_init(struct mxman *mxman, struct fwhdr_if **fw_wlan, struct fwhdr_if **fw_wpan, void *start_dram,
		      size_t size_dram)
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
		if (hdm_wifi_enable) {
			SCSC_TAG_ERR(MXMAN, "Wifi service also is prohibited by HDM.\n");
			mx140_release_file(mxman->mx, firm);
			return -EINVAL;
		}
#endif
