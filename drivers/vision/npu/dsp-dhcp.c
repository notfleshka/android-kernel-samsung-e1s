...
#endif
	{
		dsp_err("offset(%x) exceeds used size(%lx, %lx), starting at (%llx)\n",
			offset, sizeof(struct dsp_dhcp_arr),
			dsp_dhcp->dhcp_size, dsp_dhcp->dhcp_iova);
		BUG_ON(1);
	}
...
int npu_dhcp_update_imb_info(struct npu_system *system,
			u32 imbl_init_sz, u32 imbh_init_sz)
{
	struct dsp_dhcp *dhcp;
	u32 imb_str_iova, imb_end_iova;
	int ret = 0;

	dhcp = system->dhcp;

	imb_str_iova = system->imb_alloc_data[0].chunk_imb->paddr;
	imb_end_iova = system->imb_alloc_data[1].chunk_imb->paddr +
		system->imb_alloc_data[1].chunk_imb->size;

	dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_IMB_START, imb_str_iova);
	dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_IMB_END, imb_end_iova);
	dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_IMBL_SZ, imbl_init_sz);
	dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_IMBH_SZ, imbh_init_sz);
	return ret;
}
#endif
...
#define DYNAMIC_KERNEL	(2)
	...
	dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_MODE, 0);
	for (i = 0; i < NPU_MAX_PROFILE_SESSION; i++) {
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_FIRMWARE_NPU_TIME0 + (i * 2), 0);
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_FIRMWARE_DSP_TIME0 + (i * 2), 0);

		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_SUB_MAX_ITER0 + (i * 3), 0);
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_SUB_SIZE0 + (i * 3), 0);
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_SUB_IOVA0 + (i * 3), 0);

		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_MAIN_MAX_ITER0 + (i * 3), 0);
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_MAIN_SIZE0 + (i * 3), 0);
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_PROFILE_MAIN_IOVA0 + (i * 3), 0);
	}
	...
	if (rdev->system.s2d_mode == NPU_S2D_MODE_ON) {
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_RECOVERY_DISABLE, 1);
	} else {
		dsp_dhcp_write_reg_idx(dhcp, DSP_DHCP_RECOVERY_DISABLE, 0);
	}
...
int dsp_dhcp_get_master(struct npu_session *session, u32 type)
{
	...
	switch (type) {
	case NPU_HWDEV_ID_DSP:
		return dsp_dhcp_read_reg_idx(dhcp, DSP_DHCP_FIRMWARE_DSP_TIME0 + (session->uid * 2));
	case NPU_HWDEV_ID_NPU:
		return dsp_dhcp_read_reg_idx(dhcp, DSP_DHCP_FIRMWARE_NPU_TIME0 + (session->uid * 2));
	default:
